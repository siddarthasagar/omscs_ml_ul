...
report are \hl{highlighted in light yellow}.  The two new reduced-space K-selection sweep figure captions are highlighted in full.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{A3 --- Graph/Text Legibility (--2 pts)}

\textbf{Comment.} \textit{``Some of the plot sizes are quite small, making
legends, axis labels and plot values illegible.  All plots and results should
be easily readable so that the analysis can be verified.''}
...
figures were regenerated.  The two new reduced-space K-selection sweep figures
...
\textbf{Comment.} \textit{``It is not clear how the new number of clusters
were selected.  In this step, you need to re-select the optimal number of
clusters.  Plots of Silhouette, AIC/BIC should be provided to justify the
selection of optimal number of clusters.  You must provide an appropriate and
orderly demonstration/visualization of your results produced using combined DR
then Clustering across the 6 pairwise combinations for the Wine dataset.
[Same requirement for Adult dataset.]''}

\textbf{Root cause.} Step~3 reused the raw-space frozen K values ($K\!=\!2/8$,
$N\!=\!7$) in all six reduced spaces without re-running sweeps.  The report
therefore showed only final silhouette results without any selection evidence,
making the chosen cluster counts appear arbitrary.
...
\begin{itemize}
  \item \textbf{Wine reduced-space K-selection sweep (new figure):}
        3 rows (PCA/ICA/RP) $\times$ 2 cols (KMeans silhouette, GMM
        BIC+AIC).  Dashed vertical line marks selected K per panel.
  \item \textbf{Adult reduced-space K-selection sweep (new figure):}
        same layout.
\end{itemize}
...
\textbf{Comment.} \textit{``You must appropriately relate any observed speed
up in wall clock time on your EM-clustered data with the baseline NN to the
effect of EM clustering and make mention of or descriptively allude to the
filtering effect.  [Same requirement for KMeans-clustered data.]''}

\textbf{Root cause.} Wall-clock time was not collected per seed for the
cluster-feature variants, so the Step~5 results table had no timing column
and no runtime comparison with the baseline NN was possible.

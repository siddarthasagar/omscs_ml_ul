...
report are \hl{highlighted in light yellow}.  New figure captions (Figs.~8
and~9) are highlighted in full.
...
figures were regenerated.  Figs.~8--9 (new K-selection sweep figures)
...
\begin{itemize}
  \item \textbf{Fig.~8 (new):} Wine reduced-space K selection sweeps ---
        3 rows (PCA/ICA/RP) $\times$ 2 cols (KMeans silhouette, GMM
        BIC+AIC).  Dashed vertical line marks selected K per panel.
  \item \textbf{Fig.~9 (new):} Adult reduced-space K selection sweeps,
        same layout.
\end{itemize}

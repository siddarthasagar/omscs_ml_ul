...
\begin{figure}[t]
  ...
  \end{subfigure}
  \caption{K-Means metric sweep ($K\!=\!2$--$20$). \textbf{Takeaway:} Wine peaks
  sharply at $K\!=\!2$ with silhouette declining monotonically thereafter ---
  consistent with one dominant partition (red vs.\ white) rather than quality
  grades.  Adult shows a gradual plateau beginning near $K\!=\!8$; the absence
  of a clean elbow reflects the high-dimensional sparse OHE geometry where
  Euclidean distances are less discriminative.}
...
indicating two compact, well-separated macro-clusters.  Given the prior EDA
finding that quality classes are heavily overlapping, this bipartite structure
almost certainly captures the red/white split rather than any quality boundary.
Wine GMM selects $K=7$ via BIC minimization but achieves silhouette 0.040: EM
finds a statistically optimal generative decomposition, but the resulting
components do not correspond to compact spatial regions.
...
discriminative.  Adult GMM achieves a strongly negative BIC
($-8.29\!\times\!10^6$) at $K=7$, indicating an excellent generative fit; the
negative sign is expected for large-$n$ datasets where the log-likelihood term
dominates the BIC penalty.  The contrast between Adult's excellent BIC and its
low silhouette (0.058) illustrates the distinction between generative fit and
hard-assignment cluster quality: EM's soft components can overlap substantially
while still maximizing likelihood.
...
perspective.  Wine retains only 4 of 12 components above the median kurtosis
threshold, meaning most features behave nearly Gaussian after whitening --- no
strong independent non-Gaussian sources exist.  Adult retains 11 of 22
whitened components, reflecting the richer mixture of right-skewed numeric
features and binary OHE indicators whose distributions are far from Gaussian.
...
(0.112, $+180\%$).  This divergence reflects algorithm geometry.  ICA
...
\begin{figure*}[t]
  ...
  \caption{Learning curves across DR variants (median $\pm$ IQR, 10 seeds).
...
\textbf{AI Use Statement.}  I used Claude Code (Anthropic) for code generation,
debugging, and refactoring across all experimental phases, and to assist with
LaTeX structure and grammar editing throughout the report.  I used GitHub
Copilot for boilerplate code and debugging in the VS Code editor.  I reviewed,
verified, and understand all assisted content.  All experimental analysis,

...
dimensionality reduction (DR) methods --- Principal Component Analysis (PCA),
Independent Component Analysis (ICA), and Random Projection (RP) --- to the
...
for linear projection to expose.  Neural network (NN) experiments on Wine confirm
...
Structurally redundant one-hot encoding (OHE) groups motivate the hypothesis that PCA will find
...
we use Macro-F1 as the primary metric.  The \texttt{class} duplicate column was
...
assignment --- and neither Logistic Regression (LR) nor Random Forest (RF) exceeds 49\% Macro-F1, confirming a hard
multiclass boundary with no dominant separating direction.

\textbf{Hypothesis.}  The EDA above establishes two contrasting dataset geometries.
...
in Adult's sparse OHE space, and select component count by minimizing Bayesian Information Criterion (BIC)~\cite{schwarz1978}.
...
All neural network (NN) experiments use the same architecture from the OL report: one hidden
...
Table~\ref{tab:phase2} reports the final metrics at the frozen $K$ values;
Figs.~\ref{fig:kmeans_sweep} and~\ref{fig:gmm_sweep} show the full selection sweeps used to determine them.

\begin{table}[!ht]
  \caption{Raw clustering metrics at frozen $K$.
           $\uparrow$\,=\,higher is better; $\downarrow$\,=\,lower is better.
           CH\,$=$\,Calinski-Harabasz\,($\uparrow$);
           DB\,$=$\,Davies-Bouldin\,($\downarrow$);
           BIC\,$=$\,Bayesian Information Criterion\,($\downarrow$);
           AIC\,$=$\,Akaike Information Criterion\,($\downarrow$).}
  \label{tab:phase2}
...
\begin{figure}[!ht]
  ...
  \end{subfigure}
  \caption{K-Means metric sweeps across $K \in [2,20]$.
  Wine peaks sharply at $K\!=\!2$; Adult plateaus near $K\!=\!8$.}
...
\begin{figure}[!ht]
  ...
  \begin{subfigure}{\linewidth}
    ...
    \caption{Adult GMM}
  \end{subfigure}
  \caption{GMM metric sweeps across $K \in [2,20]$.
  Both datasets select $K\!=\!7$ via BIC.
  Adult's strongly negative BIC confirms an excellent generative fit.}
  \label{fig:gmm_sweep}
\end{figure}

Table~\ref{tab:phase2} and Figs.~\ref{fig:kmeans_sweep}--\ref{fig:gmm_sweep} summarize the clustering
...
\begin{figure}[!ht]
  ...
  most correlated with income --- confirming that PCA collapses the redundant
  OHE encoding (\texttt{education} vs.\ \texttt{education-num}) into a single dominant direction.  PC3 surfaces an independent capital-gains axis.}
...
\hl{\textbf{K re-selection.}  The reduced representations alter the geometric
structure of the data, so we re-select the optimal cluster count in each of the
six reduced spaces independently, using the same label-free criteria applied in
Step~1: K-Means by joint silhouette\,/\,CH\,/\,DB (highest silhouette, CH as
tiebreaker); GMM by BIC minimum.
Figs.~}\ref{fig:sweep_wine}\hl{ and~}\ref{fig:sweep_adult}\hl{ show the full selection
sweeps with the selected }$K$\hl{ marked; figure captions report the resulting values.}

\begin{figure}[!ht]
  \centering
  \includegraphics[width=\linewidth]{phase4_clustering/wine_phase4_reduced_sweeps}
  \caption{\hl{[New figure] Wine reduced-space K selection sweeps (rows\,=\,DR method; left\,=\,K-Means
  silhouette; right\,=\,GMM BIC\,+\,AIC).  Dashed vertical line marks the selected }$K$\hl{.
  }\textbf{\hl{Takeaway:}}\hl{ PCA preserves the }$K\!=\!\WinePcaReducedKmK$\hl{ red/white split;
  ICA and RP find more granular partitions at }$K\!=\!\WineIcaReducedKmK$\hl{ and
  }$K\!=\!\WineRpReducedKmK$\hl{ respectively.  GMM selects higher component counts in
  all reduced spaces, reflecting richer mixture structure exposed after projection.}}
  \label{fig:sweep_wine}
\end{figure}

\begin{figure}[!ht]
  \centering
  \includegraphics[width=\linewidth]{phase4_clustering/adult_phase4_reduced_sweeps}
  \caption{\hl{[New figure] Adult reduced-space K selection sweeps.
  }\textbf{\hl{Takeaway:}}\hl{ PCA reduces Adult K-Means to }$K\!=\!\AdultPcaReducedKmK$\hl{ (from raw }$K\!=\!8$\hl{)
  --- the compressed 22-d space consolidates OHE co-occurrence clusters.
  ICA and RP select higher K, indicating that independent-component and
  random-projection bases each expose different granular structure.
  GMM BIC selects near-maximal component counts (}$\geq\!19$\hl{) across all three DR
  methods, showing that the Adult distribution is best described as many
  overlapping components regardless of the projection used.}}
  \label{fig:sweep_adult}
\end{figure}

\begin{table}[!ht]
  \caption{Silhouette in reduced spaces \hl{at reduced-space $K$} vs.\ raw baseline.
           Bold\,=\,best per row.}
  \label{tab:phase4}
...
comparison \hl{at the reduced-space $K$ values}.  The hypothesis is directionally confirmed: Adult K-Means gains
...
\hl{The same ICA-GMM alignment appears on Adult: ICA gives GMM a }$+\AdultGmmIcaGainPct\%$\hl{ gain
(}$\AdultGmmRawSil$\hl{ to }$\AdultGmmIcaSil$\hl{), while PCA }\emph{\hl{degrades}}\hl{ Adult GMM silhouette to
}$\AdultGmmPcaSil$\hl{ (}$\AdultGmmPcaGainPct\%$\hl{ vs. raw) --- PCA's variance-maximizing axes do not
expose the soft mixture structure that GMM finds in the independent-component subspace.}
...
avoids.  \hl{ICA (}$d\!=\!4$\hl{, }$K\!=\!\WineIcaReducedKmK$\hl{) produces a moderate Wine K-Means result (}$\WineKMIcaSil$\hl{);
with its own optimal }$K$\hl{ the ICA space finds tighter components than at
}$K\!=\!2$\hl{, but its 4-d representation still limits separation relative to PCA's
8-d subspace.}
...
than a different grouping.  \hl{Wine KMeans under ICA selects }$K\!=\!\WineIcaReducedKmK$\hl{ and achieves
ARI\,=\,}$\WineKMIcaAri$\hl{ against the raw-space partition --- the assignment is substantially
different because ICA's 4-d subspace encodes independent chemical directions
that do not align with the raw-space two-cluster separation.}
...
\begin{table}[!ht]
  ...
  \label{tab:phase6}
  \resizebox{\columnwidth}{!}{\input{tables/tab_phase6_nn}}
\end{table}
...
\textbf{\hl{Speed.}}  \hl{Because cluster features are \emph{appended} to the raw 12-d
input, input dimension grows (}\KMeansOnehotDim\hl{-d for KMeans one-hot;
}\GmmPostDim\hl{-d for GMM posterior), so no reduction in first-layer computation is
expected.  Cluster-feature variants complete in }\KMeansOnehotTimingS\hl{\,s
(KMeans one-hot), }\KMeansDistTimingS\hl{\,s (KMeans distances), and
}\GmmPostTimingS\hl{\,s (GMM posterior) --- the same range as the Step~4
DR-reduced variants (}\ReducedTimingLo\hl{--}\ReducedTimingHi\hl{\,s), confirming that
appending a few extra dimensions to a 12-d input carries negligible per-epoch cost
at this dataset size.  The Step~4 raw baseline (}\RawTimingS\hl{\,s) is not directly
comparable because it includes PyTorch's one-time JIT compilation overhead on the
first forward pass of that run; after warmup, per-epoch cost differences are
}$\sim$\hl{2--3\,ms.  The performance gain from cluster augmentation therefore comes
from richer feature representation, not reduced computational cost.}
